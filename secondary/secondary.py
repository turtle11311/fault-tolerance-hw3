...
class ElectDataLoader():
    def __init__(self, db_loc: str):
        self.db_loc = db_loc
        self.Result_loc = 'secondary/electionResult.json'
...
class eVotingServer(inner_pb2_grpc.eVotingReplicaServicer):
    def __init__(self) -> None:
        self.electDB = ElectDataLoader('secondary/elections.json')
        self.Election_loc = 'secondary/elections.json'
        self.Result_loc = 'secondary/electionResult.json'
    ...
    def changeToPrimary(self):
        with grpc.insecure_channel('localhost:50051') as channel:
            try:
                ...
                status = inner_stub.ElectionRecovery(inner_pb2.Elections(elections = election))
            except grpc.RpcError as e:
                logging.error(e)
            finally:
                if status:
                    logging.info("change To Primary")
                else:
                    logging.warning("Still secondary")
                
    
    def serve(self):
        try:
            self.changeToPrimary()
